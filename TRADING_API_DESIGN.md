# Open Trader — Universal Trading API & Bot Sandbox: Design & Spec

Status: **design proposal, nothing implemented, not yet wired into the `req/` requirements
tree.** An earlier version of this document decomposed `req/engine/TRADE.yml` into child
requirement items; that was wrong — `TRADE` scopes order placement/cancellation specifically, a
narrow peer of `MARKET`/`WALLET`/`HISTORY` under `ENGINE` (the four exchange-REST-domain groupings:
market data, trading, balances, history), not the whole bot/indicator/strategy platform described
below. Where this design attaches to the requirements tree — a new top-level branch, a different
...
```
                         ...
                         │   C++ shared library — Strategy Runtime)   │
                         └───────────────┬─────────────────┬─────────┘
...
## 3.1 Market data & indicator series
...
higher period's boundary — reusing `BuoyCandleQuotes`'s own period-bucketing algorithm, so both
the base and derived series share the exact same closed/active semantics and no indicator ever
"sees the future" of an unclosed higher-timeframe candle.

## 3.2 Trigger & signal evaluation (geometry + linear programming)
...
generalises per-exchange REST calls into one method surface. This is the piece most likely to
actually live under (or alongside) the existing `TRADE` order-execution scope in `req/` once that
placement question is resolved.
...
2. **Data-shape compatibility**: candle-series accessors return the same column names Hummingbot's
   `CandlesBase` does (`timestamp, open, high, low, close, volume, …`) over Open Trader's
   `IIndicator`/candle series, so existing `candles_df.ta.rsi(...)`-style Hummingbot controller
   code needs no column renames.
...
1. Indicator series: `IIndicator` + one built-in (SMA) wired onto `BuoyCandleQuotes`.
2. Trigger evaluation: `BooleanEvaluator` first (cheapest, covers crossover-style triggers), then
...
Each of these is a substantial feature in its own right and, once the requirements-tree placement
above is settled, should go through the project's normal test-first, user-reviewed process
(`req/README.md`) rather than landing as one large change.

## Open questions

- **Where this attaches in `req/`** — see the status note at the top; unresolved.
