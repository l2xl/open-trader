...
float approx_char_px(float font_size)
{
    return std::max(1.0f, font_size * kAvgCharWidthN / kAvgCharWidthD);
}

float approx_label_px(const std::string& s, float font_size)
{
    return static_cast<float>(s.size()) * approx_char_px(font_size);
}
...
void PriceRuler::CalculateSize(InstrumentPanel& panel)
{
    ...
    const float label_w = std::max(approx_label_px(sample, font_size), kMinLabelChars * approx_char_px(font_size));
    mReservedWidth = kTickLength + kLabelPadding * 2 + static_cast<int64_t>(std::ceil(label_w));

    Rectangle& rect = panel.MutableInnerDataRect();
    rect.w = std::max<int64_t>(0, rect.w - mReservedWidth);
}
...
void PriceRuler::RebuildAll(InstrumentPanel& panel)
{
    ...
    const float axis_x       = static_cast<float>(rect.x_end()) + 0.5f * hud_px.x;
    const float y_top_hud    = canvas_h - static_cast<float>(rect.y);
    const float y_bottom_hud = canvas_h - static_cast<float>(rect.y_end());
    ...
    const double pts_per_px = static_cast<double>(top_pts - bottom_pts) / static_cast<double>(rect.h);
    ...
    // collide.
    float last_label_top = std::numeric_limits<float>::max();
    ...
    for (uint64_t tick = (bottom_pts / step + 1) * step; tick <= top_pts; tick += step) {
        ...
        const float label_center = canvas_h - hud_y;
        const float half_h       = 0.5f * font_size;
        if (label_center + half_h + kMinLabelGapPx <= last_label_top) {
            emit_label(*mLabelScene, panel.DefaultFontName(), font_size, format_price(tick, pd),
                       label_x, hud_y + half_h, 210, 210, 210);
            last_label_top = label_center - half_h;
        }
    }
